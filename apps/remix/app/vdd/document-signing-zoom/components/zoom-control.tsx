...
export const ZoomControl = ({ className, variant = 'desktop' }: ZoomControlProps) => {
  ...
  if (variant === 'desktop') {
    return (
      <div
        className={cn(
          'fixed bottom-4 right-4 z-50 hidden items-center gap-2 rounded-lg border bg-background p-2 shadow-lg lg:flex',
          className,
        )}
        role="toolbar"
  ...
  // Mobile: Sticky bar below header
  return (
    <div
      className={cn(
        'sticky top-0 z-40 flex items-center justify-center gap-2 border-b bg-background p-2 shadow-sm lg:hidden',
        className,
      )}
      role="toolbar"
      aria-label="Zoom controls"
